...
export default {
  ...
  methods: {
    handleScroll() {
      const { scrollTop, offsetHeight } = document.documentElement; // scrolltop and scrollheight set in varaible
      const { innerHeight } = window; // window inner height
      const bottomOfWindow = Math.round(scrollTop) + innerHeight === offsetHeight; // scroll height set
